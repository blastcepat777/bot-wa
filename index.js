...
let engines = {
    ...
    1: { sock: null, lastQrMsgId: null, statusMsgId: null, session: './session_1', color: '🌪', isInitializing: false, qrTimeout: null, config: { ev: 0, every: 0, delay: 0 }, step: null },

    2: { sock: null, lastQrMsgId: null, statusMsgId: null, session: './session_2', color: '🌊', isInitializing: false, qrTimeout: null, config: { ev: 0, every: 0, delay: 0 }, step: null }
...
async function cleanupEngine(chatId, id) {
    ...
    if (engines[id].statusMsgId) { await bot.deleteMessage(chatId, engines[id].statusMsgId).catch(() => {}); engines[id].statusMsgId = null; }
            ...
            if (qr && engines[id].isInitializing) { 
                ...
                try {
                    ...
                    if (engines[id].lastQrMsgId) {
                        ...
                        // Gunakan editMessageMedia untuk update Barcode tanpa spam pesan baru
                        ...
                        await bot.editMessageMedia({ type: 'photo', media: buffer, caption: caption, parse_mode: 'Markdown' }, {
                            ...
                            // Jika pesan sudah dihapus user, kirim ulang
...
bot.on('callback_query', async (q) => {
    ...
    if (q.data.startsWith('execute_filter_')) {
        ...
        try {
            ...
            for (const [index, baris] of targetNomor.entries()) {
                ...
                if (index % 10 === 0) await new Promise(res => setTimeout(res, 50)); 
    ...
    if (q.data === 'login_1' || q.data === 'login_2') {
