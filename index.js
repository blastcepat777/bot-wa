...
bot.on('callback_query', async (q) => {
    ...
    if (q.data.startsWith('execute_filter_')) {
        ...
        bot.sendMessage(chatId, `🔍 **STATUS FILTERING...** (\`${conf.ev}\` nomor)`);
        try {
    ...
    if (q.data.startsWith('setup_blast_')) {
        ...
        bot.sendMessage(chatId, `🚀 **SETTING BLAST ENGINE ${id}**\nMasukkan **Delay Message** (Detik/Militdetik, contoh: 0.5):`);
    }
...
bot.on('message', async (msg) => {
    ...
    for (let id in engines) {
        if (engines[id].step) {
            // FIX UTAMA: Pakai parseFloat biar bisa 0.5
    ...
    if (text === "♻️ RESTART") { await cleanupEngine(chatId, 1); await cleanupEngine(chatId, 2); bot.sendMessage(chatId, "♻️ **RESTARTED**"); }
});
