...
const JEDA_MS = 1000; // DIUBAH MENJADI 1 DETIK (1000ms)
...
function ambilDaftarNomor() {
    ...
    return data.split('\n')
        .map(line => {
            ...
            return {
                nama: parts[0],
                nomor: parts[parts.length - 1].replace(/[^0-9]/g, '')
            };
        })
...
async function startWA(chatId) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr && !isBlasting) {
            ...
            await bot.sendPhoto(chatId, buffer, { caption: "📸 **SCAN QR BARU**" });
        }
        ...
        if (connection === 'close') {
            ...
            if (daftar.length === 0) {
                isBlasting = false;
                return bot.sendMessage(chatId, "❌ Nomor habis atau format salah.");
            }

            bot.sendMessage(chatId, `🎉 **Terhubung!**\n🚀 Memulai Blast (Jeda 1 Detik) ke **${daftar.length}** nomor...`);
...
bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, "🔍 Menghubungkan...");
    startWA(msg.chat.id);
});
