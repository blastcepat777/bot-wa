...
let engines = {
    1: { sock: null, lastQrMsgId: null, session: './session_1', color: '🌪', config: { ev: 0 }, step: null },
    2: { sock: null, lastQrMsgId: null, session: './session_2', color: '🌊', config: { ev: 0 }, step: null }
};
...
bot.on('callback_query', async (q) => {
    ...
    if (data.startsWith('execute_filter_')) {
        ...
        try {
            ...
            bot.sendMessage(chatId, `⏳ **NGEJAM ${dataNomor.length} NOMOR...**`);
        ...
        } catch (e) { bot.sendMessage(chatId, "❌ Cek File Nomor!"); }
    }

    if (data.startsWith('lepas_jam_')) {
        const id = data.split('_')[2];
        bot.sendMessage(chatId, `🚀 **MELEPAS...**`);
        initWA(chatId, id); 
    }
});
